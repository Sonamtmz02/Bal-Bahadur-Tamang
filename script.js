...
function applyTheme(theme) {
    ...
    if (metaTheme) {
        metaTheme.setAttribute("content", safeTheme === "dark" ? "#08090c" : "#f7f4ee");
    }
...
function renderWork() {
    ...
    if (!items.length) {
        grid.innerHTML = `<p class="work-empty">${t("work.empty")}</p>`;
        return;
    }

    grid.innerHTML = items.map((item, index) => {
        const number = String(index + 1).padStart(2, "0");
        const categoryLabel = t(`filter.${item.category}`);
...
function initProfileFallback() {
    ...
    function setFallback() {
        ...
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 1000"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#181c25"/><stop offset="100%" stop-color="#08090c"/></linearGradient></defs><rect width="800" height="1000" fill="url(#g)"/><circle cx="400" cy="430" r="190" fill="#d2a869" opacity="0.16"/><text x="400" y="520" text-anchor="middle" font-family="Arial, sans-serif" font-size="170" font-weight="700" fill="#d2a869">BT</text></svg>`;
